                ...
                <div class="tab-pane fade show active" id="showtimes-pane" role="tabpanel">
                    ...
                    <div class="card border-0 shadow-sm rounded-4 mb-4">
                        <div class="card-body p-3 d-flex flex-wrap align-items-center gap-3">
                            <span class="fw-bold text-dark"><i class="fas fa-filter me-2 text-primary"></i>Filter:</span>
                            
                            <!-- Room Filter -->
                            <select class="form-select form-select-sm rounded-pill shadow-none border-secondary-subtle" id="roomFilter" style="width: auto; padding-right: 2.5rem; cursor: pointer;">
                            ...
                            <div class="btn-group" role="group">
                                <input type="radio" class="btn-check" name="timeFilter" id="timeAll" value="all" checked>
                                <label class="btn btn-sm btn-outline-light text-dark rounded-start-pill border-end-0" for="timeAll">All</label>

                                <input type="radio" class="btn-check" name="timeFilter" id="timeMorning" value="morning">
                                <label class="btn btn-sm btn-outline-light text-dark border-end-0" for="timeMorning">Morning</label>

                                <input type="radio" class="btn-check" name="timeFilter" id="timeAfternoon" value="afternoon">
                                <label class="btn btn-sm btn-outline-light text-dark border-end-0" for="timeAfternoon">Afternoon</label>
                                
                                <input type="radio" class="btn-check" name="timeFilter" id="timeEvening" value="evening">
                                <label class="btn btn-sm btn-outline-light text-dark rounded-end-pill" for="timeEvening">Evening</label>
                            </div>
                    ...
                    <?php else: ?>
                        <div class="card border-0 shadow-sm rounded-4 p-4 bg-white mb-5">
                            <ul class="nav nav-underline mb-4 border-bottom" id="dateTab" role="tablist">
                                <?php $index = 0; foreach ($data['grouped_showtimes'] as $date => $shows): ?>
                                    <li class="nav-item">
                                        <button
                            ...
                            <div class="tab-content">
                                <?php $index = 0; foreach ($data['grouped_showtimes'] as $date => $shows): ?>
                                    <div class="tab-pane fade <?php echo $index === 0 ? 'show active' : ''; ?>"
                                        ...
                                        <div class="d-flex flex-wrap gap-3" id="showtime-list-<?php echo $date; ?>">
                                            <?php foreach ($shows as $show): 
                                                ...
                                                elseif($hour < 17) $period = 'afternoon';
                                            ?>
                                                <a href="<?php echo URLROOT; ?>/booking/seats/<?php echo $show->id; ?>"
                                                   class="btn btn-outline-light text-dark border shadow-sm p-0 overflow-hidden text-start showtime-item position-relative group-hover"
                                                   ...
                                                   data-available="<?php echo $available; ?>">
                                                    ...
                                                    <div class="p-3">
                                                        ...
                                                        <div class="d-flex justify-content-between align-items-center pt-2 border-top border-light-subtle">
                                                            <span class="fw-bold text-success small"><?php echo number_format($show->price/1000); ?>k</span>
                                                            <span class="badge bg-light text-secondary border border-light-subtle fw-normal" style="font-size: 0.7rem;"><?php echo $available; ?>/<?php echo $total; ?></span>
                                                        </div>
                                                    </div>
                                                    <!-- Progress Bar at bottom -->
                                                    <div class="progress" style="height: 4px; border-radius: 0;">
                                                        <div class="progress-bar bg-<?php echo $available < 10 ? 'danger' : ($available < 30 ? 'warning' : 'success'); ?>" 
                                                             role="progressbar" style="width: <?php echo 100 - $percent; ?>%"></div>
                                                    </div>
                                                </a>
                                            <?php endforeach; ?>
                                        </div>
                    ...
                    <div class="card border-0 shadow-sm rounded-4 p-4 mt-5">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            <?php if (!isLoggedIn()): ?>
                                <a href="<?php echo URLROOT; ?>/auth/login" class="btn btn-sm btn-outline-primary rounded-pill">Login to Review</a>
                            <?php endif; ?>
                        </div>
                                ...
                                <form action="<?php echo URLROOT; ?>/comments/create" method="POST">
                                    ...
                                    <div class="mb-3">
                                        <div class="rating-css">
                                            <select name="rating" class="form-select form-select-sm w-auto d-inline-block" style="padding-right: 2.5rem;">
                                    ...
                                    <button type="submit" class="btn btn-primary rounded-pill px-4 btn-sm">Post Review</button>
                                </form>
                            </div>
                                    ...
                                    <div class="border-bottom pb-3 mb-3">
                                        <div class="d-flex justify-content-between">
                                            <div class="d-flex align-items-center mb-2">
                                                ...
                                                <div>
                                                    <span class="fw-bold d-block lh-1 text-dark" style="font-size: 0.9rem;"><?php echo $comment->user_name ?? 'User'; ?></span>
                                                    <span class="text-muted small" style="font-size: 0.75rem;"><?php echo date('d M Y', strtotime($comment->created_at)); ?></span>
                                                </div>
                                            </div>
                                            <div class="text-end">
                                                <span class="badge bg-warning text-dark me-2"><i class="fas fa-star me-1 small"></i><?php echo $comment->rating; ?></span>
                                                <?php if (isLoggedIn() && $_SESSION['user_id'] == $comment->user_id): ?>
                                                    <!-- Edit Button -->
                                                    <a href="<?php echo URLROOT; ?>/comments/edit/<?php echo $comment->id; ?>" class="btn btn-sm btn-outline-secondary border-0 rounded-circle p-1 me-1" style="width: 24px; height: 24px; line-height: 1;" title="Edit">
                                                        <i class="fas fa-pencil-alt" style="font-size: 0.8rem;"></i>
                                                    </a>
                                                    
                                                    <!-- Delete Button -->
                                                    <!-- Delete Button -->
                                                    <form id="delete-form-<?php echo $comment->id; ?>" action="<?php echo URLROOT; ?>/comments/delete/<?php echo $comment->id; ?>" method="POST" class="d-inline">
                                                        <button type="button" class="btn btn-sm btn-outline-danger border-0 rounded-circle p-1" 
                ...
                <div class="tab-pane fade" id="details-pane" role="tabpanel">
                    <div class="card border-0 shadow-sm rounded-4 p-4">
                        ...
                        <p class="text-secondary mb-5" style="line-height: 1.8;">
                           Review: The movie "<?php echo $data['movie']->title; ?>" is a masterpiece of storytelling. 
                        ...
                        <div class="ratio ratio-16x9 rounded-3 overflow-hidden shadow-sm bg-dark">
                            <iframe src="https://www.youtube.com/embed/dQw4w9WgXcQ?controls=0" title="YouTube video player" allowfullscreen></iframe>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content border-0 rounded-4 shadow">
            ...
            <div class="modal-footer border-0 justify-content-center pt-0 pb-4">
                ...
                <button type="button" class="btn btn-danger rounded-pill px-4 shadow-sm fw-bold" id="confirmDeleteBtn">Yes, Delete</button>
            </div>
        </div>
    </div>
</div>
